...
export default function Rail({ user, role }: { user: string | null; role: string | null }) {
  ...
  return (
    <aside className="relative w-[214px] shrink-0 bg-rail border-r border-line flex flex-col py-5 min-h-screen">
      ...
      <div className="px-[18px] pb-4">
        <h1 className="text-[20px] font-extrabold tracking-[0.16em]">
      ...
      {GROUPS.map((g) => (
        <div key={g.label} className="px-2.5 mt-3.5">
          <div className="text-[8px] tracking-[0.2em] text-[#3d4c5c] px-[11px] pb-1.5 uppercase font-bold">{g.label}</div>
          {g.items.map((it) => {
            ...
            return (
              <Link
                key={it.href}
                href={it.href}
                className={`flex items-center justify-between text-[12.5px] px-[11px] py-[7px] rounded mb-px ${
